...
meta <- meta %>%  select(`List ID`, Type, Birthyear, Gender, Code, Language, Glottocode, Village, District, Lat, Lon)
...
database <- words_meta %>% select(`Concept nr.`, Concept, Word, `Standardized Transcription`, Set, Village, Language, District, `List ID`, Birthyear, Gender)
...
database$`Concept nr.` <- as.character(database$`Concept nr.`)
database$`List ID` <- as.character(database$`List ID`)
database$Gender <- as.character(database$Gender)
