...
library(dendextend)
library(pvclust)
dist_no_dict <- read_tsv("convert/all_loans_dist.tsv")
...
library(dendextend)
library(pvclust)
dist_no_dict <- read_tsv("convert/all_loans_dist_strict.tsv")
row.names(dist_no_dict) <- dist_no_dict$Speaker
dist_no_dict <- dist_no_dict[,-which(grepl("D_",colnames(dist_no_dict)))]
row.names(dist_no_dict) <- dist_no_dict$Speaker
dist_no_dict <- dist_no_dict[-which(grepl("D_",row.names(dist_no_dict))),]
...
p2 <- ggplot(data = all_turkic_lezgi_mediation_counts, aes(x = InLezgi, y = Percent)) + 
    geom_jitter(aes(color = Village), position = position_jitter(width = 0.3, height = 0.1)) +
    stat_summary(fun.y = "median", geom = "point", pch = 3, group = 1, size = 5)+
...
p3 <- ggplot(data = all_turkic_avar_mediation_counts, aes(x = InAvar, y = Percent)) + 
    geom_jitter(aes(color = Village), position = position_jitter(width = 0.3, height = 0.1)) +
    stat_summary(fun.y = "median", geom = "point", pch = 3, group = 1, size = 5)+
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.height= 7}
#all_turkic_all_regions <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1),]))
#all_turkic_all_regions$Speaker <- rownames(all_turkic_all_regions)
#all_turkic_all_regions <- left_join(Districts, all_turkic_all_regions, by= c("Speaker"))
#all_turkic_all_regions <- gather(all_turkic_all_regions,Lexeme,Present, the_ant_5:the_year_2,factor_key=TRUE)
#head(all_turkic_all_regions)

#avar_all_regions <-  as.data.frame(t(d[which(d$D_Avar>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$Turk_total == 0),]))
#avar_all_regions$Speaker <- rownames(avar_all_regions)
#avar_all_regions <- left_join(Districts, avar_all_regions, by= c("Speaker"))
#avar_all_regions <- gather(avar_all_regions,Lexeme,Present, the_ant_20:the_year_11,factor_key=TRUE)
#avar_all_regions <- avar_all_regions[-which(avar_all_regions$Lexeme %in% avar_cognates$Lexeme.x),]

#georgian_all_regions <-  as.data.frame(t(d[which(d$D_Georgian>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$Turk_total == 0),]))
#georgian_all_regions$Speaker <- rownames(georgian_all_regions)
#georgian_all_regions <- left_join(Districts, georgian_all_regions, by= c("Speaker"))
#georgian_all_regions <- gather(georgian_all_regions,Lexeme,Present, the_ant_1:the_year_5,factor_key=TRUE)

#chechen_all_regions <-  as.data.frame(t(d[which(d$D_Chechen>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$Turk_total == 0),]))
#chechen_all_regions$Speaker <- rownames(chechen_all_regions)
#chechen_all_regions <- left_join(Districts, chechen_all_regions, by= c("Speaker"))
#chechen_all_regions <- gather(chechen_all_regions,Lexeme,Present, the_ant_9:the_worm_28,factor_key=TRUE)

#all_loans <- rbind(all_turkic_all_regions, avar_all_regions)
#all_loans <- rbind(all_loans, georgian_all_regions)
#all_loans <- rbind(all_loans, chechen_all_regions)
#all_loans$Code <- sapply(strsplit(as.character(all_loans$Lexeme), "_"), tail, 1)
#all_loans$Lexeme <- str_sub(all_loans$Lexeme, 1, str_length(all_loans$Lexeme)-2)
#all_loans$Code <- as.integer(all_loans$Code)*as.integer(all_loans$Present)
#all_loans$Code[all_loans$Code == 0] <- NA
#write_tsv(all_loans, "Convert/All_Loans.tsv")
...
#Mediation of Turkic Influence via Lezgian and Akhty Lezgian
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.height= 7}

all_turkic_in_both_Lezgi <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Lezgian == 1  & d$Khlut_total > 0),]))
all_turkic_not_in_Lezgi_but_in_Khlut <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Lezgian != 1 & d$Khlut_total > 0),]))
all_turkic_in_Lezgi_but_not_in_Khlut <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Lezgian == 1 & d$Khlut_total == 0),]))
all_turkic_not_in_Lezgi_and_not_in_Khlut <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Lezgian != 1 & d$Khlut_total == 0),]))
...
p2 <- ggplot(data = all_turkic_lezgi_mediation_counts, aes(x = InLezgi, y = Percent)) + 
    ...
    facet_wrap(. ~ factor(District, levels = c("Rutul", "Tsunta", "Botlikh", "Akhvakh")), nrow = 1)+
    theme(legend.position = "bottom")+
    scale_x_discrete(labels=c("In Both", "In Khlut but not in Lezgian", "In Lezgian but not in Khlut", "Not in Both"))+
    theme(legend.position = "bottom", axis.text.x = element_text(angle = -30))
ggplotly(p2)

p2 <- ggplot(data = all_turkic_lezgi_mediation_counts[which(all_turkic_lezgi_mediation_counts$District == "Rutul"),], aes(x = InLezgi, y = Percent)) + 
    geom_jitter(aes(color = Village), position = position_jitter(width = 0.3, height = 0.1)) +
    stat_summary(fun.y = "median", geom = "point", pch = 3, group = 1, size = 5)+
    labs(title = "Fig. 3. Lezgian Mediation of Turkic Influence in the Rutul District", x = "", y = "% of Loanwords")+
    theme_bw()+
    theme(legend.position = "bottom")+
    scale_x_discrete(labels=c("In Both", "In Khlut but not in Std. Lezgian", "In Std. Lezgian but not in Khlut", "Not in Both"))+
    theme(legend.position = "bottom", axis.text.x = element_text(angle = -30))+
    ylim(-5,105)
ggplotly(p2)
```

#Avar and Chechen Mediation of Turkic Influence
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.height= 7}

all_turkic_in_avar_and_in_chechen <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar == 1 & d$D_Chechen == 1),]))

all_turkic_not_in_avar_and_in_chechen <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar != 1 & d$D_Chechen == 1),]))

all_turkic_in_avar_and_not_in_chechen <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar == 1 & d$D_Chechen != 1),]))

all_turkic_not_in_avar_and_not_in_chechen <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar != 1 & d$D_Chechen != 1),]))
...
p2 <- ggplot(data = all_turkic_AC_mediation_counts, aes(x = InAC, y = Percent)) + 
    geom_jitter(aes(color = Village), position = position_jitter(width = 0.3, height = 0.1)) +
    stat_summary(fun.y = "median", geom = "point", pch = 3, group = 1, size = 5)+
    labs(title = "Fig. 3. Lezgian Mediation of Turkic Influence", x = "", y = "% of Loanwords")+
    theme_bw()+
    facet_wrap(. ~ factor(District, levels = c("Rutul", "Tsunta", "Botlikh", "Akhvakh")), nrow = 1)+
    theme(legend.position = "bottom")+
    scale_x_discrete(labels=c("In Both Avar and Chechen", "In Chechen but not in Avar", "In Avar but not in Chechen", "Not in Both"))+
    theme(legend.position = "bottom", axis.text.x = element_text(angle = -30))
ggplotly(p2)

p2 <- ggplot(data = all_turkic_AC_mediation_counts[which(all_turkic_AC_mediation_counts$District == "Botlikh"),], aes(x = InAC, y = Percent)) + 
    geom_jitter(aes(color = Village), position = position_jitter(width = 0.3, height = 0.1)) +
    stat_summary(fun.y = "median", geom = "point", pch = 3, group = 1, size = 5)+
    labs(title = "Fig. 3. Avar and Chechen Mediation of Turkic Influence in the Botlikh District", x = "", y = "% of Loanwords")+
    theme_bw()+
    theme(legend.position = "bottom")+
    scale_x_discrete(labels=c("In Both Avar and Chechen", "In Chechen but not in Avar", "In Avar but not in Chechen", "Not in Both"))+
    theme(legend.position = "bottom", axis.text.x = element_text(angle = -30))+
...
#Avar and Georgian Mediation of Turkic Influence
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 9, fig.height= 7}

all_turkic_in_avar_and_in_georgian <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar == 1 & d$D_Georgian == 1),]))

all_turkic_not_in_avar_and_in_georgian <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar != 1 & d$D_Georgian == 1),]))

all_turkic_in_avar_and_not_in_georgian <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar == 1 & d$D_Georgian != 1),]))

all_turkic_not_in_avar_and_not_in_georgian <-  as.data.frame(t(d[which(d$Turk_total>0 & d$D_Russian != 1 & d$D_Arabic != 1 & d$D_Persian != 1 & d$D_Avar != 1 & d$D_Georgian != 1),]))
...
p2 <- ggplot(data = all_turkic_AG_mediation_counts, aes(x = InAG, y = Percent)) + 
    geom_jitter(aes(color = Village), position = position_jitter(width = 0.3, height = 0.1)) +
    stat_summary(fun.y = "median", geom = "point", pch = 3, group = 1, size = 5)+
    labs(title = "Fig. 3. Lezgian Mediation of Turkic Influence", x = "", y = "% of Loanwords")+
    theme_bw()+
    facet_wrap(. ~ factor(District, levels = c("Rutul", "Tsunta", "Botlikh", "Akhvakh")), nrow = 1)+
    theme(legend.position = "bottom")+
    scale_x_discrete(labels=c("In Both Avar and georgian", "In georgian but not in Avar", "In Avar but not in georgian", "Not in Both"))+
    theme(legend.position = "bottom", axis.text.x = element_text(angle = -30))
ggplotly(p2)
...
dat[1,2] <- 1
dat[2,2] <- 1
p2 <- ggplot(data = dat, aes(x = InAG, y = Percent)) + 
    geom_jitter(aes(color = Village), position = position_jitter(width = 0.3, height = 0.1)) +
    stat_summary(fun.y = "median", geom = "point", pch = 3, group = 1, size = 5)+
    labs(title = "Fig. 3. Avar and georgian Mediation of Turkic Influence in the Botlikh District", x = "", y = "% of Loanwords")+
    theme_bw()+
    theme(legend.position = "bottom")+
    scale_x_discrete(labels=c("In Both Georgian and Avar", "In Georgian but not in Avar", "In Avar but not in Georgian", "Not in Georgian and not in Avar"))+
    theme(legend.position = "bottom", axis.text.x = element_text(angle = -30))+
    ylim(-5,105)
ggplotly(p2)
```
